...
export default function AccountPage() {
  ...
  const [siguiendoUsuarios, setSiguiendoUsuarios] = useState<FollowingUser[]>([]);
  const [loading, setLoading] = useState(true);
  const [loadingSiguiendo, setLoadingSiguiendo] = useState(false);
  ...
  const loadSiguiendo = async () => {
    ...
    if (user) {
      try {
        ...
        setSiguiendoUsuarios(data.following || []);
      } catch (error) {
              ...
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                ...
                <TabsContent value="siguiendo" className="mt-6">
                  {loadingSiguiendo ? (
                    ...
                    <FollowingList users={siguiendoUsuarios} />
                  )}
                </TabsContent>
