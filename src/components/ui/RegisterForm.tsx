...
export default function RegisterForm() {
  ...
  async function onSubmit(values: z.infer<typeof registerSchema>) {
    ...
    try {
      ...
      if (res.redirected) {
        alert.show({ title: 'Registro exitoso', description: 'Revisa tu correo para confirmar', variant: 'default' })
        setTimeout(() => {
          window.location.href = res.url
        }, 500)
        return
      }
      alert.show({ title: 'Error', description: 'No se pudo completar el registro', variant: 'destructive' })
      setLoading(false)
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-5">
              ...
              {field.value && (
                <div className="text-sm text-gray-600 mt-1">
                  <div className="flex flex-wrap gap-2">
                    <span className={`text-xs ${field.value.length >= 8 ? 'text-green-600' : 'text-red-500'}`}>
                      {field.value.length >= 8 ? '✓' : '✗'} 8+ caracteres
                    </span>
                    <span className={`text-xs ${/[A-Z]/.test(field.value) ? 'text-green-600' : 'text-red-500'}`}>
                      {/[A-Z]/.test(field.value) ? '✓' : '✗'} Mayúscula
                    </span>
                    <span className={`text-xs ${/[a-z]/.test(field.value) ? 'text-green-600' : 'text-red-500'}`}>
                      {/[a-z]/.test(field.value) ? '✓' : '✗'} Minúscula
                    </span>
                    <span className={`text-xs ${/[0-9]/.test(field.value) ? 'text-green-600' : 'text-red-500'}`}>
                      {/[0-9]/.test(field.value) ? '✓' : '✗'} Número
                    </span>
                    <span className={`text-xs ${/[#?!@$%^&*-]/.test(field.value) ? 'text-green-600' : 'text-red-500'}`}>
            ...
            <FormItem>
              ...
              <FormControl>
                <CustomInput
                  ...
                  className={form.formState.errors.confirmPassword ? 'border-red-500 focus:border-red-500' : ''}
                  {...field}
                />
              </FormControl>
              <FormMessage />
              ...
              {field.value && form.watch('password') && (
                <div className="text-sm text-gray-600 mt-1">
                  <span className={`text-xs ${field.value === form.watch('password') ? 'text-green-600' : 'text-red-500'}`}>
        ...
        <CustomButton
          ...
          className={(!form.formState.isValid && Object.keys(form.formState.errors).length > 0) ? 'opacity-50 cursor-not-allowed' : ''}
        >
          {loading ? (
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="text-xs text-gray-500 mt-2 p-2 bg-gray-100 rounded">
            ...
            <div>Dirty Fields: {Object.keys(form.formState.dirtyFields).join(', ') || 'none'}</div>
          </div>
        )}
        <p className="self-center leading-2 text-slate-700">
          ¿Ya tienes cuenta? <Link className="font-semibold text-sky-500" href='./login'>Iniciar sesión</Link>
        </p>
      </form>
    </Form>
  </>
